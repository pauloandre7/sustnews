<h1> Sustenews </h1>
<p align="center">
  <img loading="lazy" src="https://img.shields.io/badge/Status-Em Desenvolvimento-blue"/>
  <img loading="lazy" src="https://img.shields.io/badge/UTFPR-ebd834"/>
  <img loading="lazy" src="https://img.shields.io/badge/Sustentabilidade-green"/>
</p>

<h2>Descrição do Projeto</h2>
<p>
  Projeto em desenvolvimento para disciplina Desenvolvimento Web Front-End do curso de Engenharia de Software da Universidade Tecnológica Federal do Paraná. O Sustnews
  é um site para agregar notícias sustentáveis do paraná e compartilhar ações comunitárias/sustentáveis da cidade de cornélio procópio, facilitando o acesso à informações
  sobre políticas de sustentabilidade e oportunidades de voluntariado.
</p>

<h2>Acesso ao Projeto</h2>
<p>
  Github Pages: https://pauloandre7.github.io/sustnews/
</p>

<h2>Tecnologias Utilizadas</h2>

- `Html`
- `CSS3`
- `JavaScript`

<h2>Autores</h2>

| Autor |
| :---: |
| [<img loading="lazy" src="https://avatars.githubusercontent.com/u/147359503?v=4" width=115><br><sub>Paulo André Oliveira</sub>](https://github.com/pauloandre7) |
